...
public class OAuth2AuthenticationSuccessHandler implements AuthenticationSuccessHandler {
  ...
  public OAuth2AuthenticationSuccessHandler(JwtTokenProvider jwtTokenProvider, UserRepository userRepository,
                                            TokenService tokenService, OAuth2Properties oAuth2Properties) {
  ...
  public void onAuthenticationSuccess(HttpServletRequest request, HttpServletResponse response, Authentication authentication) throws IOException, ServletException {
    ...
    // 로그 기록 추가
    logUserActivity(user.getId(), deviceId);
  ...
  private void logUserActivity(Long userId, String deviceId) {
    String logMessage = String.format("userId: %s, deviceId: %s, timestamp: %s", userId, deviceId, Instant.now());
