...
//        when(commentRepository.save(any(Comment.class))).thenReturn(Comment.builder().id(1L).writer(writer).user(user).content("test content").build());
//
//        // when
...
//
//    @Test
//    void 댓글수정_요청자가대상자_불일치할때실패() {
//        // given
//        User writer = User.builder().id(2L).build();
//        Comment comment = Comment.builder().id(1L).writer(writer).content("old content").build();
//        CommentRequest request = new CommentRequest(2L, "new content");
//
//        when(commentRepository.findById(1L)).thenReturn(Optional.of(comment));
//
//        // when & then
//        assertThrows(BusinessException.class, () -> commentService.updateComment(1L, request));
//    }
//
//    @Test
//    void 댓글삭제성공() {
